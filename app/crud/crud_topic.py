...
from app.models.m_topic import TopicMetaDB
from app.schemas.s_topic import TopicMetaCreate, TopicMetaUpdate
...
class CRUDTopic(CRUDBase[TopicMetaDB, TopicMetaCreate, TopicMetaUpdate]):
    def get_meta_by_topic_uniq_id(
        self, db: Session, *, topic_uniq_id: UUID
    ) -> TopicMetaDB:
        result = (
            db.query(TopicMetaDB)
            .filter(TopicMetaDB.topic_uniq_id == topic_uniq_id)
            .first()
        )
        return result


crud_topic = CRUDTopic(TopicMetaDB)
