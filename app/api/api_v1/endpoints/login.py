...
from app.schemas import s_token


router = APIRouter()


@router.post("/login/access-token/", response_model=s_token.Token)
def login_access_token(
    ...
    """
    user = crud_user.crud_user.authenticate(
        db, email=form_data.username, password=form_data.password
    )
    if not user:
        raise HTTPException(status_code=400, detail="Incorrect email or password")
    elif not crud_user.crud_user.is_active(user):
...
@router.post("/login/test-token", response_model=s_user.User)
def test_token(current_user: m_user.UserDB = Depends(deps.get_current_user)) -> Any:
    """
    Test access token
    """
    return current_user
