...
inner join (
	...
	LEFT JOIN commentars ON questions.commentar_id =commentars.id 
	group by 
		questions.id, topic_question.topic_id,
		read_texts.read_text, read_texts.created_at, read_texts.updated_at,
		records.filename, records.created_at, records.updated_at,
		commentars.commentar, commentars.created_at, commentars.updated_at
) temp3_question_data
